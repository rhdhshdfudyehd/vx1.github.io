...
function showPurchaseModal() {
    ...
    const paymentElement = $('.payment-option.selected .payment-name');
    const payment = paymentElement.length ? paymentElement.text() : 'USDT (TRC20)';
    $('#summaryEmail').text(email);
    $('#summaryQuantity').text(quantity);
    $('#summaryTotal').text(total);
    $('#summaryPayment').text(payment);
    $('#modalPaymentName').text(payment);
    const address = payment.includes('TRC20') ? 'TXrzgVmz7bKXxke3KgtEANvN2PagVQEHfA' : '0x5cA1D8Ee36B92ED120b7Dc689967A66E09f7579C';
    $('#usdtAddress').text(address);
...
$(document).ready(function () {
    ...
    $('body').css('opacity', '0');
    setTimeout(() => {
        $('body').css({'transition': 'opacity 0.5s ease', 'opacity': '1'})
    }, 100)
});